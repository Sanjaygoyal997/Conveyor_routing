...
- `DBM_EQUIPMENT_NOT_IN_MASTER` flags equipment that balanced tires at DBM in the window but is not a DBM-area row in `master.equipment_master`.
- Setting a running rim is refused when `equipment_master` puts the machine in another area than the rim (e.g. a DBM rim on a TUO machine).
